...
// for threads
#define NR_THREADS 8
sem_t *semStart[NR_THREADS];
sem_t *semStop[NR_THREADS];

static char strSemStart[NR_THREADS][15];
static char strSemStop[NR_THREADS][15];
...
static threadInfo* tI[NR_THREADS];
...
femShallowProblem *femShallowCreate(const char *meshFileName)
{
	...
	/*
	 for (i = 0; i < myProblem->mesh->nNode; ++i) {
	...
	//printf("myProblem->mesh->nLocalNode %d\n", myProblem->mesh->nLocalNode);
...
void tsunamiCompute(double dt, int nmax, int sub, int order, const char *meshFileName, const char *baseResultName){
	...
	//------------------------------------------------------------------------------------
	// 1 - Opening sempahores for integration
	
	for (i=0; i<NR_THREADS; i++) {
		sprintf(strSemStart[i], "%d", i);
		semStart[i]=sem_open(strSemStart[i],O_CREAT,S_IRUSR | S_IWUSR,0);
		if(semStart[i]==SEM_FAILED) {
			printf("sem_open failed");
		}
		sprintf(strSemStop[i], "%d", i+NR_THREADS);
		semStop[i]=sem_open(strSemStop[i],O_CREAT,S_IRUSR | S_IWUSR,0);
		if(semStop[i]==SEM_FAILED) {
			printf("sem_open failed");
		}
	}
	
	//------------------------------------------------------------------------------------
	// 2 - Creating the threads for Integration over elements
	
	for(i = 0; i < NR_THREADS; i++){
		// Fuck les free de ce truc, ça prend rien comme place !
		tI[i] = malloc(sizeof(threadInfo));
		tI[i]->theProblem = theProblem;
		tI[i]->threadNr = i;
		tI[i]->nmax = nmax;
		if(pthread_create(&(threadIDs[i]), NULL, &femComputeThread, tI[i]))
	...
	for(i = 0; i < NR_THREADS; i++){
		...
		free(tI[i]);
		err=sem_close(semStart[i]);
		if(err!=0) {
			printf("sem_close failed");
		}
		err=sem_unlink(strSemStart[i]);
		if(err!=0) {
			printf("sem_unlink failed");
		}
		err=sem_close(semStop[i]);
		if(err!=0) {
			printf("sem_close failed");
		}
		err=sem_unlink(strSemStop[i]);
		if(err!=0) {
			printf("sem_unlink failed");
		}
	}
...
void* femShallowAddIntegralsElements(void* args)
{
    ...
    for (elem=threadNr*nElem/NR_THREADS; elem < (threadNr+1)*nElem/NR_THREADS; elem++) {
        ...
        for (k=0; k < theRule->n; k++) {
			...
			double lat = asin(z3d/R)*180.0/M_PI;
...
void* femShallowAddIntegralsEdges(void* args)
{
    ...
    for (edge=threadNr*nEdge/NR_THREADS; edge < (threadNr+1)*nEdge/NR_THREADS; edge++) {
        ...
        for (k=0; k < theRule->n; k++) {
			...
			/*
			 x = (interpolate(phiEdge,X,mapEdge[0],2)+interpolate(phiEdge,X,mapEdge[1],2))/2.0;
			 y = (interpolate(phiEdge,Y,mapEdge[0],2)+interpolate(phiEdge,Y,mapEdge[1],2))/2.0;
			 h = (interpolate(phiEdge,Z,mapEdge[0],2)+interpolate(phiEdge,Z,mapEdge[1],2))/2.0;
			 */
