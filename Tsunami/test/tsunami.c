...
void femShallowComputeTest(femShallowProblem *theProblem);
void* femComputeThread1(void* args);
void* femComputeThread2(void* args);
...
void tsunamiCompute(double dt, int nmax, int sub, int order, const char *meshFileName, const char *baseResultName){
	...
	int i;
	for (i=1; i<=nmax; i++) {
		femShallowComputeTest(theProblem);
		if (i%sub == 0) {
...
void femShallowComputeTest(femShallowProblem *theProblem)
{
	...
	for(i = 0; i < NR_THREADS; i++){
		...
		tI->theProblem = theProblem;
		tI->threadNr = i;
		if(pthread_create(&(threadIDs[i]), NULL, &femComputeThread1, tI))
			printf("error in thread creation\n");
	}
	for(i = 0; i < NR_THREADS; i++){
		pthread_join(threadIDs[i], NULL);
	...
	for(i = 0; i < NR_THREADS; i++){
		...
		tI->theProblem = theProblem;
		tI->threadNr = i;
		if(pthread_create(&(threadIDs[i]), NULL, &femComputeThread2, tI))
			printf("error in thread creation\n");
	}
	for(i = 0; i < NR_THREADS; i++){
		pthread_join(threadIDs[i], NULL);
