...
{
	int nElem,nNode,nLocal,i,j,index;
    double *H,*E,*U,*V;
    ...
    do {
        ...
        if (frame0 != frame) {
			...
			GLfloat colors[nLocal*3], coord[nLocal*3];
			
            for (i=0; i < nElem; ++i) {
                for (j=0; j < nLocal; ++j) {
                    index = theMesh->elem[nLocal*i+j];
                    ...
                    double x = theMesh->X[index];
                    double y = theMesh->Y[index];
                ...
                glDrawArrays(GL_POLYGON, 0, nLocal);
				...
				*/
                ...
                glEnableClientState(GL_VERTEX_ARRAY);
                for (j=0; j < 9; ++j)
					coord[j] = coord[j] * 1.004;
                glVertexPointer(3, GL_FLOAT, 0, coord);
                glDrawArrays(GL_LINE_LOOP, 0, nLocal);
                glDisableClientState(GL_VERTEX_ARRAY); }
            glfwSwapBuffers();
