...
void guiSandbox(double dt, int nmax, int sub, int order, const char *meshFileName, const char *baseResultName){
    ...
    do {
        ...
        if (frame0 != frame) {
			...
			printf("mouseX : %d, mouseY : %d",mouseX,mouseY); //mouse = 389;
			...
			*/			
			...
			if(iDontTrustThisFrameThing <= 1){
				...
				// negative in X : goes right in the sphere
				// positive in Z : goes down in the sphere
				double decalageX = minX+(maxX-minX)/2.0;
				double decalageY = minY+(maxY-minY)/2.0;				
				double decalageZ = minZ+(maxZ-minZ)/2.0;
				printf("Decalages : %f, %f and %f\n",decalageX,decalageY,decalageZ);
			...
			GLfloat colorsOrder2[3*4], coordOrder2[3*4];
			// Pour chaque élément
            for (elemNr=0; elemNr < nElem; ++elemNr){
				...
				if(order == 1){
					...
					for (j=0; j < nLocalNode; ++j){
						...
						if((colors[j*3] != 0 || colors[j*3+1] != 0) && colors[j*3+2] != 1){
							minX = MIN(coord[j*3+0],minX); maxX = MAX(coord[j*3+0],maxX);
							minY = MIN(coord[j*3+1],minY); maxY = MAX(coord[j*3+1],maxY);
							minZ = MIN(coord[j*3+2],minZ); maxZ = MAX(coord[j*3+2],maxZ);
						} else if(colors[j*3+2] < 1){
							//printf("red and green : %f and %f\n",colors[j*3],colors[j*3+1]);
						}
				...
				// Mais en temps de guerre, tout est permi !
					...
					int triangleData[12] = {5,0,3,
					                        2,5,4,
					                        4,3,1,
											3,4,5};
					int squareData[16] = {8,6,3,7,
					                      8,5,2,6,
					                      8,4,1,5,
					                      8,7,0,4};
					int start = 0;
					...
					if(nLocalNode == 3){
						for(start = 0; start < 4; start++){
							...
							for(j = 0; j < 3; j++){
								...
								//printf("start : %d, j : %d - result : %d\n",start,j,squareData[start*4+j]);
								index = spaceN*elemNr+triangleData[start*3+j];
								...
								// RGB pour le noeud en question
						        colorsOrder2[j*3+0] = 3.5*(value)*(value);
						        colorsOrder2[j*3+1] = (1-value)*(value)*3.5;
						        colorsOrder2[j*3+2] = (1-value)*(1-value);
								 ...
								 */
						        double Factor = (4*R*R + x*x + y*y)*(R/6);
						        coordOrder2[j*3+0] = 4*R*R * x / Factor;
						        coordOrder2[j*3+1] = 4*R*R * y / Factor;
						        coordOrder2[j*3+2] = (4*R*R - x*x - y*y)*R / Factor;
							...
							// On spécifie comment va être l'élément à dessiner
						    glVertexPointer(3, GL_FLOAT, 0, coordOrder2);
						    glNormalPointer(GL_FLOAT, 0, coordOrder2);
						    glColorPointer(3, GL_FLOAT, 0, colorsOrder2);
							// On dessine, en précisant quand-même que l'élément est un quadrilatère
							glDrawArrays(GL_TRIANGLES, 0, 3);
						...
						for(start = 0; start < 4; start++){
							...
							for(j = 0; j < 4; j++){
								...
								//printf("start : %d, j : %d - result : %d\n",start,j,squareData[start*4+j]);
								index = spaceN*elemNr+squareData[start*4+j];
    ...
    do {
        ...
        if (frame0 != frame) {
			...
			*/			
            ...
            glTranslatef(0.0f,9.0f,0.0f);
            double tt = 0;
            glRotatef(0.3f*(GLfloat)401 + (GLfloat)tt*10.0f,0.0f,0.0f,1.0f);
			glRotatef(0.3f*(GLfloat)142 + (GLfloat)tt*10.0f,0.0f,-1.0f,0.0f);
